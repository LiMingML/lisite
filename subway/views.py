...
def index(request):
    ...
    if request.method == "POST":
        ...
        # 使用更安全的get方法，并设置默认值
        vendre_adjuste = float(data.get("vendre_adjuste", 0))
        vendre_par_cartes = float(data.get("vendre_par_cartes", 0))
        debourse = float(data.get("debourse", 0))
        depot = float(data.get("depot", 0))

        # 货币面值配置（可扩展）
        denominations = [
            ('005', 0.05), ('01', 0.1), ('025', 0.25),
            ('1', 1), ('2', 2), ('5', 5), ('10', 10),
            ('20', 20), ('50', 50), ('100', 100)
        ]
        ...
        # 统一处理货币计算
        for suffix, value in denominations:
            field_name = f"d{suffix}"
            if field_name in data:
                count = int(data[field_name])
                context[field_name] = count
                amount = count * value
                total += amount
                context[f"res_{field_name}"] = f"{amount:.2f}"

        # 计算应存款项
        depot_a_faire = vendre_adjuste - vendre_par_cartes - debourse - depot
        context.update({
            "vendre_adjuste": vendre_adjuste,
            "vendre_par_cartes": vendre_par_cartes,
            "debourse": debourse,
            "depot": depot,
            "depot_a_faire": f"{depot_a_faire:.2f}",
            "total": f"{total:.2f}"
        })
